...
//Rare exceptions to indicate a bug in the _allowance code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_allowance = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_allowance]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.allowance.module"
